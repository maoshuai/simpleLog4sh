...
# 日志文件目录
if [ x$SIMPLE_LOG_4_SH_DIR = "x" ];then
	SIMPLE_LOG_4_SH_DIR=/tmp/simplelog4sh # 避免不同的用户部署时公用文件
fi
...
getLogFile()
{
	if [ ! -e $SIMPLE_LOG_4_SH_DIR ];then
		mkdir -p $SIMPLE_LOG_4_SH_DIR
	fi 
	local logDate=$(date +"%Y%m%d")
	local todayLogFile=$SIMPLE_LOG_4_SH_DIR/log_${logDate}.log
